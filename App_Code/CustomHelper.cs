...
namespace System.Web.Mvc
{
    public static class CustomHelper
    {
        private static string GetErrorMessage(ModelMetadata metadata)
        {
            ...
            ComponentModel.ICustomTypeDescriptor customTypeDescriptor = new AssociatedMetadataTypeTypeDescriptionProvider(metadata.ContainerType).GetTypeDescriptor(metadata.ContainerType);
            if (customTypeDescriptor != null)
            {
                ComponentModel.PropertyDescriptor descriptor = customTypeDescriptor.GetProperties().Find(metadata.PropertyName, true);
                RequiredAttribute req = (new List<Attribute>(descriptor.Attributes.OfType<Attribute>())).OfType<RequiredAttribute>().FirstOrDefault();
             ...
             Expression<Func<TModel, TValue>> expression) // lambda expression signature
        {
            ...
            Array enumList = Enum.GetValues(Nullable.GetUnderlyingType(metadata.ModelType) ?? metadata.ModelType);
